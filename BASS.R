...
library(minpack.lm) # for 'nlsLM' function which is used for modeling 'Bass model'

require(propagate) # for forecasting

# === Bass Model

train <- inputVector
time = 1:length(train)
BassFormula <- train ~ (M * ( ((P+Q)^2 / P) * exp(-(P+Q) * time) ) /(1+(Q/P)*exp(-(P+Q)*time))^2)

Bass.nlSLM <- nlsLM(BassFormula , start = list( M = max(train), P = 0.1, Q = 0.1))
(Bass.nlSLM <- update(Bass.nlSLM))

# === Predict by Bass Model

newData <- seq(1, 182, by = 1)
newData <- data.frame(time = newData) # colnames should be 'time' to run 'predictNLS' function
Bass_predicted <- predictNLS(Bass.nlSLM, newdata = newData)
